...
class VideoTracker(object):
    def __init__(self, args):
        ...
        print("Using webcam " + str(0))
        self.vdo = cv2.VideoCapture(0)
        print("Using webcam " + str(1))
        self.vdo2 = cv2.VideoCapture(1)
    ...
    def __enter__(self):
        ...
        print('Camera ...')
        ret1, frame1 = self.vdo.read()
        ret2, frame2 = self.vdo2.read()
        assert ret1, "Error: Camera 1 error"
        assert ret2, "Error: Camera 2 error"
        ...
        if self.args.save_path:
            ...
            self.writer2 = cv2.VideoWriter(self.save_video_path, fourcc,
                                          self.vdo2.get(cv2.CAP_PROP_FPS), (self.im_width, self.im_height))
    ...
    def run(self):
        ...
        yolo_time2, sort_time2, avg_fps2 = [], [], []
        t_start2 = time.time()
    
        idx_frame2 = 0
        last_out2 = None
        while self.vdo.grab() & self.vdo2.grab():
            # Inference *********************************************************************
            t0 = time.time()
            _, img0 = self.vdo.retrieve()
            _, img1 = self.vdo2.retrieve()
            ...
            if idx_frame2 % self.args.frame_interval == 0:
                outputs2, yt2, st2 = self.image_track(img1)        # (#ID, 5) x1,y1,x2,y2,id
                last_out2 = outputs2
                yolo_time2.append(yt2)
                sort_time2.append(st2)
                print('Frame %d Done. YOLO-time:(%.3fs) SORT-time:(%.3fs)' % (idx_frame2, yt2, st2))
            else:
                outputs2 = last_out2  # directly use prediction in last frames
            t1 = time.time()
            avg_fps.append(t1 - t0)
            avg_fps2.append(t1 - t0)
            # post-processing ***************************************************************
            # visualize bbox  ********************************
            if len(outputs) > 0:
                bbox_xyxy = outputs[:, :4]
            ...
            if len(outputs2) > 0:
                bbox_xyxy2 = outputs2[:, :4]
                identities2 = outputs2[:, -1]
                img1 = draw_boxes(img1, bbox_xyxy2, identities2)  # BGR

                # add FPS information on output video
                text_scale = max(1, img0.shape[1] // 1600)
                cv2.putText(img1, 'frame: %d fps: %.2f ' % (idx_frame2, len(avg_fps2) / sum(avg_fps2)),
                        (20, 20 + text_scale), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255), thickness=2)
            # display on window ******************************
            if self.args.display:
                cv2.imshow("cam1", img0)
                cv2.imshow("cam2", img1)
        ...
        print('Avg YOLO time (%.3fs), Sort time (%.3fs) per frame' % (sum(yolo_time2) / len(yolo_time2),
                                                            sum(sort_time2)/len(sort_time2)))
        t_end = time.time()
        print('Total time (%.3fs), Total Frame: %d' % (t_end - t_start, idx_frame))
        print('Total time (%.3fs), Total Frame: %d' % (t_end - t_start, idx_frame2))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--camera2", action="store", dest="cam2", type=int, default="-1")
