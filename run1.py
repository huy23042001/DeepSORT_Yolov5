...
class VideoTracker(object):
    ...
    def __enter__(self):
        ...
        self.im_width2 = int(self.vdo2.get(cv2.CAP_PROP_FRAME_WIDTH))
        self.im_height2 = int(self.vdo2.get(cv2.CAP_PROP_FRAME_HEIGHT))
    ...
    def detect(self, vdo, yolo_time, sort_time, avg_fps):
        idx_frame = 0
        last_out = None
        while vdo.grab():
            # Inference *********************************************************************
    ...
    def run(self):
        ...
        t_start = time.time()
        t1 = threading.Thread(target=self.detect, args=(self.vdo, yolo_time, sort_time, avg_fps,))
        t2 = threading.Thread(target=self.detect, args=(self.vdo2, yolo_time, sort_time, avg_fps,))
        t1.start()
        t2.start()
        t1.join()
